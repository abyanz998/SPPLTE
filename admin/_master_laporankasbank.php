            ...
            <?php
              ...
              while ($kas = mysqli_fetch_array($sql_akunKas)) {
                ...
                $saldo_keluar += ($saldoKasKeluar['totalKeluar'] + $saldoBayarHutang['totalBayarKeluar']);
                 ...
                 $sis = mysqli_query($koneksi,"SELECT *
                                      FROM kas 
                                      JOIN siswa ON kas.SiswaId = siswa.idSiswa
                                      WHERE jenis_bayar.idTahunAjaran='$idTahunAjaran' AND  idAkunKas='$kas[idAkun]' AND (DATE(kas.tanggal) BETWEEN '$tgl_mulai' AND '$tgl_akhir')");
                $sw = mysqli_fetch_array($is);
                ...
                $sqlTagihanBulanan = mysqli_query($koneksi,"SELECT 
                                                              ...
                                                              WHERE jenis_bayar.idTahunAjaran='$idTahunAjaran' AND tagihan_bulanan.statusBayar='2' AND tagihan_bulanan.idAkunKas='$kas[idAkun]' AND (DATE(tagihan_bulanan.tglBayar) BETWEEN '$tgl_mulai' AND '$tgl_akhir')");
