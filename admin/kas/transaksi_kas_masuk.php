<?php
	...
	if ($tipe == 'tampil_data'){
        ...
        mysqli_query($koneksi,"INSERT INTO log_transaksi(tanggal,modul,aksi,info,penulis,browser,os,ip_address) VALUES ('$waktu_sekarang','Kas Masuk','Input Transaksi','$info','$idUsers','$browser_ok','$user_os','$ip')");
		...
		$info = 'No. Ref:'.$noref;
        mysqli_query($koneksi,"INSERT INTO log_transaksi(tanggal,modul,aksi,info,penulis,browser,os,ip_address) VALUES ('$waktu_sekarang','Kas Masuk','Pembatalan Transaksi','$info','$idUsers','$browser_ok','$user_os','$ip')");
		...
		while ($r = mysqli_fetch_array($cek_data)){
			...
			$query = mysqli_query($koneksi,"INSERT INTO kas(jenis,tipe,tanggal,idUnitSekolah,noRefrensi,idAkunKas,idKodeAkun,idTahunAjaran, keterangan,keterangan_kas,nominal,idPajak,idUnitPos,total, stdel,cby,cdate) VALUES ('Masuk','Kas Masuk','$tanggal_kas','$id_unit','$noref','$id_akun_kas','$r[idAkunBiaya]','$id_tahun_ajaran','$r[keterangan]','$keterangan_kas','$r[nominal]','$r[idPajak]','$r[idUnitPos]','$total','0','$idUsers','$tanggal_sekarang')");
			mysqli_query($koneksi,"DELETE FROM kas_transaksi WHERE noRefrensi='$noref'");

			$info = 'No. Ref:'.$noref;
        	mysqli_query($koneksi,"INSERT INTO log_transaksi(tanggal,modul,aksi,info,penulis,browser,os,ip_address) VALUES ('$waktu_sekarang','Kas Masuk','Simpan Transaksi','$info','$idUsers','$browser_ok','$user_os','$ip')");
			...
			$title = 'Input '.$r['keterangan'];
       		mysqli_query($koneksi,"INSERT INTO log_kasir(tanggal,jenisBayar,idBayar,modul,aksi,noRefrensi,nis_nip,title,nominal,penulis,browser,os,ip_address) VALUES ('$waktu_sekarang','Kas','$ambil_id[idKas]','Kas Masuk','Simpan Transaksi','$noref',null,'$title','$r[nominal]','$idUsers','$browser_ok','$user_os','$ip')");
