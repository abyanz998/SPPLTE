		...
		<li class="header">Menu</li>
			<?php
				...
				while($menu=mysqli_fetch_array($users_hak_akses_main_menu)){
					if ($_GET['view']==$menu['viewMenu']) { $aktifMainMenu = 'active'; }
					else{ $aktifMainMenu = ''; }

					if ($menu['viewMenu'] != '-'){
						echo '<li class="'.$aktifMainMenu.'"><a href="index-siswa.php?view='.strtolower($menu['viewMenu']).'"><i class="'.$menu['iconMenu'].' text-stock"></i> <span>'.ucwords($menu['namaMenu']).'</span></a></li>';
					}else{
						echo '<li class="treeview '.$aktifMainMenu.'">
							...
							while($submenu1=mysqli_fetch_array($users_hak_akses_sub_menu1)){
								...
								if ($_GET['view']==$submenu1['viewMenu']) { $aktifSub1 = 'active'; }
								else{ $aktifSub1 = ''; }

								if ($submenu1['viewMenu'] != '-'){
									echo '<li class="'.$aktifSub1.'"><a href="index-siswa.php?view='.strtolower($submenu1['viewMenu']).'"><i class="'.$submenu1['iconMenu'].'"></i> '.ucwords($submenu1['namaMenu']).'</a></li>';
								}else{
									...
									echo '<ul class="treeview-menu">';
										...
										while($submenu2=mysqli_fetch_array($users_hak_akses_sub_menu2)){
											if ($_GET['view']==$submenu2['viewMenu']) { $aktifSub2 = 'active'; }
											else{ $aktifSub2 = ''; }

											echo '<li class="'.$aktifSub2.'"><a href="index-siswa.php?view='.strtolower($submenu2['viewMenu']).'"><i class="'.$submenu2['iconMenu'].'"></i> '.ucwords($submenu2['namaMenu']).'</a></li>';
										}
